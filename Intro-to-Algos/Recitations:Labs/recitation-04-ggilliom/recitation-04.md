...
**Names:** Ila Keshishian, Garrett Giliom, Ryan Park, Ethan Sollender, Chris Callahan  
...
Taking n to be the number of words in an entire doc, this solution solves the problem in O(n^2) time, so it requires a higher runtime.  Additionally, it is not parallelizable, as for-loops are inherently sequential, so this implentation would not benifit from an infinite amount of processors.
